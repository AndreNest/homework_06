# Сделать Предыдущее ДЗ с использование функций
...
def install_ships_bot(ships_1, ships_2, max_coordinate):
    ...
    while ships_1 > 0:
        ...
        if my_list_bot[coordinate_1][coordinate_2] == 0:
            ...
            ships_1 -= 1
    while ships_2 > 0:
        coordinate_1 = random.randint(0, max_coordinate - 2)
        coordinate_2 = random.randint(0, max_coordinate - 2)
        if my_list_bot [coordinate_1][coordinate_2] == 0:
            my_list_bot[coordinate_1][coordinate_2] = 2
            my_list_bot[coordinate_1][coordinate_2 + 1] = 2
            ships_2 -= 1
    print_arr(my_list_bot)
    return my_list_bot
...
s = int(input('Сколько будет однопалубных кораблей? '))
s_2 = int(input('Сколько будет двупалубных? '))
x = int(input('Какой размерности будет поле? '))
game_battle_ships(install_ships_bot(s, s_2, x), install_ships_player(s,s_2,x), s, s_2)
